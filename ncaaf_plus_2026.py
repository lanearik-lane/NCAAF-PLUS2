...
def smart_predict(away, home, neutral=False, week=0):
    ...
    if sp_gap>20:
        ...
        txt=f"{fnm} SP+ edge of +{sp_gap:.1f} pts. {'Home field adds 2.5 pts.' if not neutral else 'Neutral site.'} Line moving toward {fnm}."
        pos=[f"{fnm} SP+ edge","ATS trend"]; neg=["Cover variance"]
    elif sp_gap>4:
        txt=f"Competitive matchup — SP+ gap {sp_gap:.1f} pts. {'Home field is the difference.' if not neutral else 'Neutral site makes this a true toss-up.'} Lean {fnm} with low conviction."
...
st.markdown(
    ...
    "<div style='font-size:.7rem;color:#f59e0b;font-weight:700'>"+now.strftime("%A, %B %-d, %Y")+"</div>"
...
def render_div(div):
    ...
    for g,p in filtered:
        ...
        st=g.get("status","scheduled"); a_sc=g.get("away_score",""); h_sc=g.get("home_score","")
        if st=="STATUS_FINAL" and a_sc and h_sc:
            score_p=f"<span class='score-final'>FINAL: {g['away']} {a_sc}–{h_sc} {g['home']}</span><br>"
        elif "IN_PROGRESS" in st and a_sc and h_sc:
        ...
        if h2h:
            ...
            rows="".join(
                ...
                f"<span class='h2h-w {'h2h-away' if m['winner']==g['away'] else 'h2h-home'}'>{m['winner']}</span>"
        ...
        globals()['st'].markdown(row+ai+h2h_html+half_html, unsafe_allow_html=True)
